 ...
 * measures *text nodes*, so it cannot reach a control's boundary and cannot reach
 * a `::before`/`::after` glyph, because a pseudo-element is not an element and
 * owns no text node.
 *
 * This repo's only previous 1.4.11 check, `e2e/border.spec.ts`, was the exact
 * shape that motivated this file: it queried `.text-input, .select-input` —
 * precisely the set `--color-control-border` was written for and correctly
 * applied to — and compared each element's border against its OWN background
 * rather than against the surface around it. Pointing a check only at the place a
 * rule is already kept is the same as not having it. It found nothing, twice per
 * run, for as long as it existed. Every BUTTON-shaped control on this page went
 * unmeasured, and two of them were failing: the six `.panel-tab`s at 1.41:1 dark
 * / 1.27:1 light, and `.btn--stop` — the only control with no fill at all — at
 * 1.26:1 dark / 1.18:1 light.
 *
 * TWO SEPARATE CHECKS LIVE HERE.
 *
 * 1. CONTROL BOUNDARIES, 3:1. A control has to be distinguishable from what
 *    surrounds it. The rule is deliberately narrow to stay useful: an element is
 *    only judged if it is *trying* to draw itself as a control — it has an own
 *    background, or a border, or an outline. A plain text link carries its own
 *    identification and is not flagged.
 *
 ...
 *      - the BORDER against that outside surface (a border is only a boundary if
 *        you can see it against what is beyond it).
 ...
 *    reports the pseudo's `content`, `color`, `background` and font, so the ink is
 *    knowable; what is not knowable is its box, because a pseudo-element has no
 *    `getBoundingClientRect`. It is measured against its host's composited
 *    backdrop, which is right for the overwhelmingly common case of a glyph
 *    painted inside its host. That approximation is stated rather than hidden: an
 *    absolutely-positioned pseudo that escapes its host is measured against the
 *    wrong surface, and `escaped` is reported so triage can see it.
 *
 *    On this page that check currently has nothing to find, and that was measured
 *    rather than assumed: `styles/main.css` has exactly two `::before`/`::after`
 *    selectors — the universal `box-sizing` reset and the universal
 *    reduced-motion clamp — and neither sets `content`. The check runs in every
 *    driven state anyway, so the first decorative glyph anyone adds is measured
 *    on arrival instead of discovered later by hand.
 *
 * Both checks reuse the composite model from `contrast.ts` — translucent colours
 * over a geometry-aware ancestor walk, with the root's canvas background beneath
 * — because a control's fill can be an `rgba()` or a `color-mix()` over a panel
 * that is itself translucent, and reading the declared value would measure
 * something the page never paints. `gate.ts` excludes the shared Crypto Lab top
 * bar from this audit, with the measurement and the reason written down there.
 */
...
export interface NonTextResult {
  /** Every control the boundary audit actually judged. A gate that measures zero
   *  controls and reports no failures is indistinguishable from one that works,
   *  so the count is returned and asserted rather than assumed. */
  controlsMeasured: number;
  failures: NonTextFailure[];
}

export async function auditNonText(
  page: Page,
  within = 'body *',
  exclude?: string
): Promise<NonTextResult> {
  return page.evaluate(([rootSelector, excludeSelector]) => {
    interface RGBA {
      r: number;
      g: number;
      b: number;
     ...
     * page's `body`: a gradient that runs the full height of a document several
     * screens tall puts text near the top on a different surface than text near
     * the bottom. So each gradient is *sampled* at the element's real location
     ...
     * (`url()`, `none`) is unmeasurable and paints nothing. This lab paints every
     * surface with a flat colour today, so the sampler is currently exercising
     * only its `backgroundColor` path — it is kept whole because the alternative
     * is a helper that quietly mismeasures the first gradient anyone adds, and
     * because the same code is what `contrast.ts` has been exercised against.
     */
     ...
     *
     * A driven pass here walks six exhibit panels, and — the expensive part —
     * hex output rendered one `<span>` per byte: sixteen `.byte-cell`s per grid,
     * three `.xor-row`s of sixteen, and a `.cipher-block` of sixteen per
     * ciphertext block. Those are hundreds of sibling elements all re-walking the
     * same ancestors up to `<body>`. Without the caches the pass re-reads the
    ...
    // oracles cannot drift apart in how they resolve a colour. Not every helper
    // in it is reachable from the two checks below; these `void`s keep
    // `noUnusedLocals` quiet without deleting code that would then have to be
    // re-derived the moment a gradient or an SVG figure appears on this page.
    void splitTopLevel;
    void over;
    void fade;
    void luminance;
     ...
     * what a page paints. Elsewhere in this fleet a lab whose dark canvas is two
     * radial gradients reported `rgba(0,0,0,0)` for both `html` and `body`; the
     * walk therefore found nothing opaque, fell through to WHITE, and reported a
     * bright cyan disclosure triangle at 3.95:1 against a page that is actually
     * near-black. The tell was that the number came out IDENTICAL in both themes
     * — a ratio that cannot depend on the theme is not measuring the theme. This
     * is the same class of mistake as the canvas-background bug in `contrast.ts`,
     * so it borrows that file's gradient-aware sampler wholesale rather than
     * approximating it a second time.
     */
    ...
    const excluded = (el: Element): boolean => !!excludeSelector && !!el.closest(excludeSelector);
    ...
    return { controlsMeasured, failures };
  }, [within, exclude ?? ''] as const);
}
