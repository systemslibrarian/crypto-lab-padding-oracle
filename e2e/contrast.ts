 ...
 *
 * This exists because axe is not a complete contrast oracle. Two classes of text
 * never reach the `violations` array a gate asserts on:
 *
 *  - text over a surface axe declines to resolve. On this page that is the hero
 *    aside, `.cl-hero-why`, whose fill is
 *    `color-mix(in oklab, var(--color-primary) 6%, transparent)` over the page
 *    background — `getComputedStyle` reports the `color-mix()` unchanged and axe
 *    files the two paragraphs inside it under `incomplete`.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Here that is `.cl-hero-sub` at
 *    `.85` and `.btn:disabled` at `.45`.
 *
 * So: walk every element that owns text, composite the real painted result
 * (translucent colours, gradient stops and opacity groups included), and compute
 * the ratio against the surface the text is genuinely sitting on rather than
 * against white. A gradient is judged at the text's own position on it.
 *
 * Opacity is modelled the way the compositor actually does it: an element with
 * `opacity < 1` renders its subtree into a group, then composites the group over
 * the backdrop. That means the *text* and the *background beside it* fade onto
 * the same backdrop independently — which is why both are carried through the
 * walk as a pair rather than fading the foreground alone.
 *
 ...
 * actually intersects the text's box; a partial intersection still counts, so the
 ...
 *
 * Three things this page forced on top of that, each of which otherwise makes the
 * helper report a ratio nothing on screen has:
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. Here the clipping
 *    boxes are `.byte-grid` and `.xor-display` (`overflow-x: auto` around rows
 *    that are 416px and 689px wide), both `.comparison-table-wrapper`s, the
 *    `.diagram-container`, `.panel-tabs-wrapper`, and `.exploit-card`
 ...
 *    failure.
 *
 *  - TRANSPARENT TEXT PAINTS NOTHING. Anything drawn `color: transparent` lays no
 *    ink down; compositing a zero-alpha foreground just returns the backdrop and
 *    reports a fixed 1:1.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. This lab
 *    draws its figures — the CBC flow diagram, the byte grids, the block
 *    visualisation — in HTML and CSS grid rather than SVG, so the sibling walk
 *    finds nothing to do: the only SVG on the page is the shared header's two
 *    `aria-hidden` marks and the footer's GitHub glyph, none of which carries
 *    text. It is kept because the alternative is a helper that silently
 *    mismeasures the first labelled diagram anyone adds.
 *
 * TWO THINGS THIS WALK CANNOT SEE, stated so neither is mistaken for coverage.
 *
 * FIRST, generated content. The walk iterates ELEMENTS, and a `::before`/`::after`
 * is not one — nor is it one to axe's `color-contrast` rule. On this page that is
 * a gap with nothing in it, and that was checked rather than assumed:
 * `styles/main.css` contains exactly two `::before`/`::after` selectors, the
 * universal `box-sizing` reset and the universal reduced-motion clamp, and
 * neither sets `content`. `nontext.ts` measures generated content anyway, in
 * every state, so the day one appears it is measured rather than discovered.
 *
 * SECOND, `aria-hidden` text. It is skipped here for the same reason axe skips
 * it, and the cost is a real blind spot, so what this lab hides was enumerated
 * rather than assumed: the `→`/`↩` flow arrows, the `vs` separators between
 * oracle badges, the severity icons on the exploit cards, the `.byte-cell__value`
 * hex pairs (whose value is re-stated in the `.sr-only` state label beside them),
 * the `.cipher-block` titles and hex runs (re-stated by the block's own
 * `role="img"` label), and the step numerals. Every one is either decorative or
 * duplicated in text that IS measured. No verdict, no recovered plaintext and no
 * query count is inside an `aria-hidden` subtree — which is the one place a live
 * readout can hide from a whole accessibility gate, since both oracles stop
 * looking there.
 */
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * is `.byte-grid` and `.xor-display`, both `.comparison-table-wrapper`s, the
     * `.diagram-container` and `.panel-tabs-wrapper` (all `overflow-x: auto`),
     * and `.exploit-card` (`overflow: hidden`). `.sr-only` is handled separately
     * below.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // footer's GitHub glyph, none of which contains text, and the hamburger is
      // built from three stroke-only <line>s, which is exactly the shape that
      // guard exists for. The rule is the same one that would apply the moment
      // this lab grows an SVG figure of its own.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `checkVisibility()` catches what the `display`/rect tests above cannot:
      // a subtree hidden by `content-visibility`, or by the `hidden` attribute on
      // an ancestor, for which Chromium may keep the last laid-out geometry. That
      // is load-bearing here — five of the six exhibit panels carry `hidden` at
      // any moment, and each of them has been laid out at least once. The gate
      // reaches every panel by clicking its tab, which is the route a reader has,
      // rather than stripping `hidden` from all six at once — which is what the
      // gate this replaces did, before its single scan.
      ...
      //
      // This lab uses the idiom for real, which is why the guard is here and not
      // merely inherited. `styles/main.css` defines `.sr-only` as
      // `position:absolute; width:1px; height:1px; clip:rect(0,0,0,0)`, and the
      // page is full of it: `#aria-announcer` is an `aria-live` region whose text
      // changes on every action, each of the sixteen `.byte-cell`s carries a
      // `.byte-cell__state-label` that is the non-colour cue for its state, and
      // the step numerals and exploit-card meta each carry an `.sr-only` prefix.
      // Every one has a rect and passes `checkVisibility()` while painting
      // exactly zero pixels; measuring them reports ratios for ink that was never
      // laid down, and `#aria-announcer` would report a fresh one on every scan.
     ...
     * `color-contrast` rule skips it — and this arithmetic oracle exists to catch
     * what axe *misses* among exposed text (gradients, opacity), not to be
     * stricter than axe on decorative content. Honour the same boundary.
     *
     * The boundary cuts both ways and is a known gap: text that is `aria-hidden`
     * but still painted is skipped by BOTH oracles, so what this lab hides was
     * enumerated rather than assumed. The list is in the header of this file.
     * Every member of it is either decorative or duplicated in text that IS
     * measured, and the one class that carries real values — the
     * `.byte-cell__value` hex pairs — takes its ink from `.byte-cell`, whose own
     * `.sr-only` state label is measured on the same surface, so the ratio is
     * covered even though the element is skipped.
     */
